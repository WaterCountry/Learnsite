...
/** readCookies  by wenzhou*/

/** Binds functions to each of the buttons, nav links, and related. */
Ardublockly.bindActionFunctions = function() {
  // Navigation buttons
  Ardublockly.bindClick_('button_upload', Ardublockly.uploadXmlFile);
  //Ardublockly.bindClick_('button_load', Ardublockly.loadUserXmlFile);
  Ardublockly.bindClick_('button_save', Ardublockly.saveSketchFile); //Ardublockly.saveXmlFile);
 ...
 */
 ...
 Ardublockly.sketch_Name=function() {
   var readname=document.getElementById('sketch_name').value;
   if(readname==''){
      var mydate=new Date();
      readname="sketch_Name"+mydate.getMilliseconds();;
   }
    return readname;
 }
 ...
 Ardublockly.saveXmlFile = function() {
  Ardublockly.saveTextFileAs(
      Ardublockly.sketch_Name()+ '.xml',
      Ardublockly.generateXml());
};

Ardublockly.uploadXmlFile = function() {
  var sketch_name=Ardublockly.sketch_Name();
  Ardublockly.saveOnlineAs(
      sketch_name,
      Ardublockly.generateXml(),Ardublockly.generateArduino());
  Materialize.toast(sketch_name+'.xml 文档 保存成功！',2000,'rounded');
};
...
/**
 * 自动保存xml格式文档到服务器上
 * @param  {[type]} fileName 文档名
 * @param  {[type]} content  内容数据
 */
Ardublockly.saveOnlineAs=function(fileName,xmlcontent,arduiocontent){
  var saveurl="saveXml.ashx";
  var spitword="##WENZHOU%WARMTOWN##";
  var allcontent=xmlcontent+spitword+arduiocontent;
...
Ardublockly.renderContent = function() {
  ...
  var xmlcontent=Ardublockly.generateXml();
  if (arduinoCode !== Ardublockly.PREV_ARDUINO_CODE_) {

  // If Code haved been changed, save button would be show. by wenzhou.
  if(Ardublockly.saved) {
    $("#button_upload").show();
    Ardublockly.saved=false;//wenzhou save cookie
  }

    Ardublockly.saveSessionStorageBlocks(); //wenzhou save cookie
  ...
  document.getElementById('content_xml').value = xmlcontent;//Ardublockly.generateXml();
};
