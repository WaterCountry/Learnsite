(function () {
	...
	var isdraw=false;	
    ipythonExample.prototype.cvshow = function (code) {
		var obj=$("#cv").find("canvas")[1]; 
		if(code.indexOf("turtle")>-1&&code.indexOf("import")>-1){
			obj=true;
		}
		if(obj){
			if(!isdraw){
				console.log("有绘图");//刚开始画，显示背景网格
				$("#cv").css('opacity', 0);//先不显示
				$("#cv").css("background-image","url(../code/grididle.png)");
				$("#cv").stop().animate({opacity: '1'},1000);
				isdraw=true;
			}			
		}
		else{
			console.log("无绘图");
			isdraw=false;
			$("#cv").css("background-image","");
		}				
    };
		
    ipythonExample.prototype.savesession = function () {
        var ln=codes.length;
    ...
    ipythonExample.prototype.getsession = function () {
        var codestr=sessionStorage.getItem(sessionkey);
		console.log("缓存代码\n",codestr);
        if(codestr!=null && codestr!=""){	 
			this.inCell.setValue(codestr);
